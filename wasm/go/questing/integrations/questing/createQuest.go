...
func CreateQuest(this js.Value, args []js.Value) interface{} {
	...
	handler := js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		...
		go func() {
			...
			enrollmentJson, err := createQuest(oracle, questData)
			if err != nil {
				errorConstructor := js.Global().Get("Error")
...
func createQuest(oracle solana.PublicKey, questData QuestResponse) ([]byte, error) {
	rpcClient := rpc.New(utils.NETWORK)

	instructions := make([]solana.Instruction, 0)
	txJson := []byte("{}")

	inferIndex := func() bool {
		questsPda, _ := quests.GetQuests(oracle)
		if questsData := quests.GetQuestsData(rpcClient, questsPda); questsData == nil {
			return true
		}
		return false
	}()

	enableQuestsIx := ops.EnableQuests(rpcClient, oracle)
	if inferIndex == true {
		instructions = append(instructions, enableQuestsIx)
	}

	createQuestRecordIx, _ := ops.CreateQuest(
		rpcClient,
		oracle,
		questing.Quest{
